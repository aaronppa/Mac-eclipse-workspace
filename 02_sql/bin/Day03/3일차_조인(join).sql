...
/*
     ...
     여러개의 테이블을 조인할 때 증거조건을 명시하지 않은 경우 M * N의 결과를 반환
*/
select employee_id, last_name, employees.department_id, department_name
from employees, departments;  


-- 양쪽 테이블에 존재하는 컬럼을 출력하려 할때는 테이블명.컬럼명 으로 알려줘야 한다.
...
-- 쿼리의 복잡함을 해결하자... 
-- 테이블 별칭 사용 : from 테이블명 별칭, from 테이블명 as 별칭(X) 
...
-- 각 사원의 이름과 직무아이디, 직무에서 받을 수 있는 최대 급여액을 출력하시오
select  e.last_name, 
          e.job_id, 
          j.max_salary
 from  employees e, 
...
-- Canada 에 근무하는 사원의 이름,  부서명, 도시명을 출력하시오
...
rename  tb_salgrade to salgrade;
  
-- "=" 이외의 연산자를 사용하는 Join을 NonEqui 조인이라 한다.
-- 사원명, 급여, 급여등급 출력   
select e.last_name, e.salary, s.grade
  from employees e, salgrade s
 where e.salary between s.min_sal and s.max_sal;  
 
-- Self 조인 : 물리적으로 하나의 테이블을 논리적으로 두개이상으로 사용하는 것...
...
-- Outer 조인 : 조인조건을 만족하지 않더라도 결과에 포함
-- 연산자 : (+)
-- 모자른쪽에 연산자를 표시함....
-- 양쪽이 모자른 경우 사용이 불가능함...
select  e.employee_id, 
...
-- inner join ~ on : equi, nonequi, self join
...
where e.last_name like 'K%' 
 ...
 -- 사원의 직무 변동 기록을 화면에 출력합니다.
-- 출력하는 내용은 사원번호, 사원명, 직무아이디, 직무명, 직무시작일, 직무 종료일 입니다.
select e.employee_id, 
         e.last_name, 
         jh.job_id, 
         j.job_title, 
